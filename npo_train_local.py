...
class EnhancedNPOLoss(nn.Module):
    ...
    def get_patch_mask(self, segmentation_masks, latent_size):
        ...
        for mask in segmentation_masks:
            # mask may be a tensor or numpy array — normalise to numpy
            if isinstance(mask, torch.Tensor):
                mask = mask.cpu().numpy()

            h, w = mask.shape
            patch_h = max(h // latent_h, 1)
            patch_w = max(w // latent_w, 1)

            downsampled = np.zeros((latent_h, latent_w), dtype=np.float32)
            for i in range(latent_h):
                for j in range(latent_w):
                    ...
                    downsampled[i, j] = 1.0 if np.any(patch > 0) else 0.0

            patch_labels.append(downsampled)

        return torch.tensor(np.stack(patch_labels), dtype=torch.float32)

    def forward(self, noise_pred_pos, noise_pred_neg, noise_target, segmentation_masks):
        # Cast to fp32 for numerical stability — avoids fp16 overflow under mixed precision
        ...
        patch_labels = self.get_patch_mask(
            segmentation_masks, (latent_h, latent_w)
        ...
        artifact_pixels = patch_labels_expanded.sum(dim=[1, 2, 3])
        ...
        # Clamp before logsigmoid: as the model improves, sample_loss_pos → 0
        # causing log_ratio → +inf, which overflows fp16 and produces NaN
...
WANDB_API_KEY = "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" # ← replace with your actual key
...
def run_validation():
    ...
    with torch.no_grad():
        for batch in val_dataloader:
            ...
            seg_masks       = batch["segmentation_mask"]   # list / tensor of (H,W)
            ...
            # convert batched tensor masks to a list of numpy arrays for loss_fn
            masks_np = [seg_masks[i].cpu().numpy() for i in range(seg_masks.shape[0])]

            loss, npo_loss, recon_loss = loss_fn(noise_pred_pos, noise_pred_neg, noise, masks_np)
...
def init_wandb(epochs, lr, beta, artifact_weight):
    """Initialise a W&B run and log all hyperparameters."""
    wandb.init(
        project="npo-artifact-removal",
        name=f"npo_synthscars_lr{lr}_b{beta}",
        config={
            "epochs":          epochs,
            "learning_rate":   lr,
            "beta":            beta,
...
print("\n" + "=" * 70)
print("STARTING ENHANCED NPO TRAINING  (local SynthScars dataset)")
print("=" * 70)
print(f"Epochs: {EPOCHS}  |  LR: {LEARNING_RATE}  |  Beta: {BETA}  |  Artifact weight: {ARTIFACT_WEIGHT}")
print("=" * 70 + "\n")

init_wandb(EPOCHS, LEARNING_RATE, BETA, ARTIFACT_WEIGHT)
global_step = 0
...
for epoch in range(EPOCHS):
    ...
    for batch in pbar:
        with accelerator.accumulate(unet):
            pixel_values  = batch["pixel_values"]
            seg_masks     = batch["segmentation_mask"]   # (B, H, W)
            pos_input_ids = batch["pos_input_ids"]
            neg_input_ids = batch["neg_input_ids"]

            with torch.no_grad():
                # NOTE: pixel_values are already in [-1, 1] from Normalize([0.5],[0.5])
                latents = vae.encode(pixel_values).latent_dist.sample() * 0.18215
            ...
            # loss_fn expects a list of numpy (H,W) masks
            masks_np = [seg_masks[i].cpu().numpy() for i in range(seg_masks.shape[0])]

            loss, npo_loss, recon_loss = loss_fn(
                noise_pred_pos, noise_pred_neg, noise, masks_np
            )

            accelerator.backward(loss)

            if not torch.isfinite(loss):
                print(f"[WARNING] Non-finite loss ({loss.item()}) at step {global_step}, skipping batch")
                optimizer.zero_grad()
            else:
    ...
    if (epoch + 1) % 2 == 0:
        ...
        accelerator.unwrap_model(unet).save_pretrained(f"./npo_checkpoint_epoch_{epoch + 1}")
...
accelerator.wait_for_everyone()
accelerator.unwrap_model(unet).save_pretrained("./npo_artifact_free_unet_final")
...
print("  UNet:         ./npo_artifact_free_unet_final")
